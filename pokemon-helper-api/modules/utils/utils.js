...
export const filterNeedCards = (data) => {
  ...
  const needCards = data
    ...
    .map((item) => {
      ...
      // Если пустая строка
      if (!expansion || !name || !number || !variant) return item;
      // Если нет дополнения
      if (!haveCards[expansion]) return item;
      // Есть нет карты
      if (!haveCards[expansion][number]) return item;
      // Если нет варианта
...
export const addCardsPrices = (data) => {
  const {
    first: { byExpansion: haveCards },
  } = getParsedContent();
  ...
  const newPricesData = pricesData.map((item) => {
    let [empty, expansion, name, number, variant, rarity, count, price] =
      item.split(",");
    let comment = "";
  ...
  const notExistPrices = values
    .filter((item) => {
      ...
      if (!!num && !!num2 && Number(num) > Number(num2)) return false;
      return !newPricesData.find(
        (i) => i.includes(item.number) && i.includes(item.variant)
      );
    })
    .map((item) => {
      return [
